...
app.post("/pylon-webhook", async (req, res) => {
	...
	// Field names/values here match the actual Pylon trigger payload:
	// { event: "assigned", ticket_id, title, status, url }
	// No assignee check is needed server-side — the trigger's own
	// "Issue assigned to [you]" condition already means this endpoint is
	// only ever called for tickets assigned to you.
	const { event, ticket_id, title, url, status } = req.body ?? {};
	...
	try {
		switch (event) {
			case "assigned":
				...
				break;
			case "closed":
				// removeTicket is a no-op for any ticket_id that was never
				// added, so this is safe even if you point a broader
				// "status changed" trigger at this same endpoint later.
				await removeTicket({ issue_id: ticket_id });
				break;
			default:
				console.warn(`Unrecognized event "${event}" (status: ${status}), ignoring.`);
		}
