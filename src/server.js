 ...
 *
 *      Trigger "Ticket assigned to me" (When: Issue Assigned, If: Assignee is you):
 *        {
 ...
 *        }
 *      The "If: Assignee is you" trigger condition means this webhook only
 *      ever fires for your tickets in the first place; assignee_id is sent
 *      anyway so the server can double-check (see PYLON_ASSIGNEE_ID below)
 *      in case the trigger condition is ever loosened or misconfigured.
 *
 ...
 *          "issue_id": "{{issue.id}}"
 *        }
 *      This fires for any closed ticket regardless of assignee — it's a
 *      no-op removal for tickets that were never added (see removeTicket).
 *
 * Gather side:
...
const { GATHER_INBOX_URL, PYLON_WEBHOOK_SECRET, PYLON_ASSIGNEE_ID, PORT = 3000 } = process.env;
...
app.post("/pylon-webhook", async (req, res) => {
	...
	const { event, issue_id, title, url, assignee_id } = req.body ?? {};

	try {
		switch (event) {
			case "issue.assigned":
				// Pylon's trigger condition should already scope this to your
				// tickets, but double-check server-side in case that condition
				// is ever loosened — a mismatch here means "assigned to someone
				// else," not "assigned to me," so skip it rather than piling it up.
				if (PYLON_ASSIGNEE_ID && assignee_id && String(assignee_id) !== PYLON_ASSIGNEE_ID) {
					console.warn(`Ignoring issue.assigned for ${issue_id}: assignee ${assignee_id} != PYLON_ASSIGNEE_ID`);
					break;
				}
				await addTicket({ issue_id, title, url });
				break;
			case "issue.closed":
				// No assignee check needed: removeTicket is a no-op for any
				// issue_id that was never added (never assigned to you), so a
				// closed ticket that wasn't yours just does nothing here.
